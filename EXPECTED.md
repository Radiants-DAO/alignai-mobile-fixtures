...
Every app is its own component (platform `expo`). Each component produces exactly one action (`action-1`) and one
source-check record per rule, rule version `mobile-wiring@1/<rule>`. The machine-readable version is
[`expected.json`](expected.json). Source-check ids and receipt digests depend on the audited commit, so they are not listed.

Repository totals: 11 components, 55 source checks — 36 pass, 9 fail, 7 inconclusive, 3 not-applicable, 0 not-run.
...
- When `setup-connect` fails, the other capability rules for that action are reported `inconclusive` with `unsupported-shape`; the analyzer does not judge network, signing or recovery behaviour on an unestablished session. Those two apps also get a skipped component-level `solana-mobile-wiring` coverage check.
...
- Component wiring coverage check: none (analysis had no gaps)
...
- SDK: `@solana-mobile/mobile-wallet-adapter-protocol-web3js`, declared `2.3.0`, resolved `2.3.0`, presence `imported`
- Inventory check: `completed`
- Component wiring coverage check: `skipped` (`unsupported-shape`)
